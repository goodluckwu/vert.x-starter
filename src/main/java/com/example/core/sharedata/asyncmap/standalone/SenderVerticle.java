      ...
      sharedData.<String, String>getLocalAsyncMap("mymap", res -> {
        if(res.succeeded()){
          ...
          map.put("foo", "bar", resPut -> {
            if(resPut.succeeded()){
              log.info("getLocalAsyncMap put foo succeed");
            }else{
              log.error("getLocalAsyncMap put foo failed!");
            }
          });
          map.put("foo2", "bar2", resPut -> {
            if(resPut.succeeded()){
              log.info("getLocalAsyncMap put foo2 succeed");
            }else{
              log.error("getLocalAsyncMap put foo2 failed!");
            }
          });
        }else{
          log.error("getLocalAsyncMap failed!");
        }
      });
      ...
      sharedData.<String, String>getAsyncMap("mymap", res -> {
        if(res.succeeded()){
          ...
          map.put("foo3", "bar3", resPut -> {
            if(resPut.succeeded()){
              log.info("getAsyncMap put foo3 succeed");
            }else{
              log.error("getAsyncMap put foo3 failed!");
            }
          });
          map.put("foo4", "bar4", resPut -> {
            if(resPut.succeeded()){
              log.info("getAsyncMap put foo4 succeed");
            }else{
              log.error("getAsyncMap put foo4 failed!");
            }
          });
        }else{
          log.error("getAsyncMap failed!");
        }
      });
